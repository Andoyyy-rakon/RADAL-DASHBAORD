...
const Acknowledge = () => {
  ...
  // Purpose: Only display reports where response_bool is true (It means they have already been responded to).
...
const handleResponse = (report, responseCode, responseBool) => {
  // Purpose: Intercept the status change click to request confirmation first.
...
const handleDelete = (id) => {
  ...
  setConfirmDeleteData({ isOpen: true, id });
};

const executeDelete = async () => {
  const { id } = confirmDeleteData;
  if (!id) return;

  try {
    // Purpose: Call the custom-built delete endpoint to purge ONLY the alert without touching the `userInfo` family database.
    // Auth token is required for delete routes, so we use axiosPrivate instead of apiaxio
    const res = await axiosPrivate.delete('/users/events/delete/' + id);
    if(res.data.success) {
    ...
    setConfirmDeleteData({ isOpen: false, id: null });
  }
};
...
useEffect(()=>{
  ...
  setreports(prev => {
    ...
    if (index !== -1) {
      ...
      // If the actual physical status changed (e.g. from SAFE to ALERT), update time.
      // Otherwise, keep the old time since it's just an admin response or duplicate signal.
    ...
    <div className='flex-1 min-h-screen bg-[#F5F7FB] dark:bg-slate-900 transition-colors duration-300'>
        <div className='p-4 sm:p-6 lg:p-8'>
            <div className='flex gap-2 items-center mb-1'>
                ...
                <h1 className='text-3xl font-semibold text-[#1F2937] dark:text-white'>Acknowledged</h1>
            </div>
                ...
                <div className="flex flex-col xl:flex-row items-start xl:items-end gap-6 mb-8">
                    <div className="bg-white dark:bg-slate-800 p-5 min-w-[240px] rounded-2xl shadow-lg relative border border-slate-100 dark:border-slate-700 transition-all">
                        <div className="flex justify-between items-center mb-3">
                            <span className="text-gray-500 dark:text-gray-400 font-medium">Acknowledged Alerts</span>
                        ...
                        <p className="text-4xl font-bold text-slate-800 dark:text-white mt-1">{filteredReports.length}</p>    
                    </div>
                </div>
            ...
            <ConfirmModal 
              ...
              message="Are you sure you want to change the response status for this alert?"
            ...
            <ConfirmModal 
              ...
              confirmColor="bg-red-500 hover:bg-red-600 text-white shadow-red-500/25"
